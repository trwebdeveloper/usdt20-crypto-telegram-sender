#!/bin/bash
echo "🔧 Transaction confirmation ve fee tracking ekleniyor..."
...
class TronService {
  ...
  // Transaction confirmation bekleme fonksiyonu
  async waitForConfirmation(txHash, maxWaitTime = 120000) {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      try {
        ...
        if (txInfo && txInfo.id) {
          ...
          const result = txInfo.result || 'UNKNOWN';
  ...
  async sendUsdt(fromPrivateKey, toAddress, amount) {
    try {
      ...
      logger.info('Confirmation bekleniyor...');

      // Confirmation bekle (maksimum 2 dakika)
      const confirmResult = await this.waitForConfirmation(txHash, 120000);

      if (confirmResult.confirmed && confirmResult.success) {
        logger.info(`✅ Transaction confirmed! Fee: ${confirmResult.fee} TRX`);
        ...
        return {
          ...
          status: 'confirmed',
          fee: confirmResult.fee,
          energyUsed: confirmResult.energyUsed,
          blockNumber: confirmResult.blockNumber
        };
      } else if (confirmResult.confirmed && !confirmResult.success) {
        throw new Error(`Transaction failed on blockchain: ${confirmResult.result}`);
      } else if (confirmResult.timeout) {
        // Timeout ama transaction hash'i ver
        logger.warn('Transaction confirmation timeout, ama hash mevcut');
        return {
          txHash: txHash,
          from: fromAddress,
          to: toAddress,
          ...
          status: 'pending',
          fee: 0,
          note: 'Confirmation timeout - İşlem hâlâ işleniyor olabilir'
        };
      } else {
        throw new Error('Transaction confirmation başarısız');
      }
...
echo "✅ Transaction confirmation ve fee tracking eklendi!"
