#!/bin/bash
echo "🔧 Telegram edit message hatası düzeltiliyor..."

cat > src/bot/handlers/wallet.js << 'WALLETEOF'
  ...
  static checkBalances() {
    return async (ctx) => {
      try {
        ...
        const loadingMsg = await ctx.reply('🔄 Bakiyeler kontrol ediliyor, lütfen bekleyin...');
        ...
        await ctx.reply('❌ Bakiyeler alınamadı. Lütfen tekrar deneyin.');
      }
    };
  }
...
echo "✅ Telegram edit message hatası düzeltildi!"
