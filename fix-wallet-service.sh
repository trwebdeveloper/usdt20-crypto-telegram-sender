#!/bin/bash
echo "🔧 Wallet Service user_id hatası düzeltiliyor..."
...
class WalletService {
  ...
  // Private key'i çöz ve transfer yap
  async sendUsdt(telegramId, walletId, toAddress, amount, masterPassword) {
    try {
      const userId = await this.getOrCreateUser(telegramId);
...
echo "✅ Wallet Service düzeltildi!"
